...
#read in data----
merged_DEseq_data <- read_csv("merged_DESeq2.csv") #your experimental data you want to fit

most_abundant_transcripts <- read_csv("most_abundant_transcripts_IDs.csv")

feature_properties <- read_csv(file = "gencode.vM27.pc_transcripts_filtered_feature_properties.csv") #df of features/parameters to fit data 
...
feature_properties %>%
  ...
  select(-c("transcript", "gene_sym", "Human_gene_name")) %>%
  mutate(Signal_seq = factor(Signal_seq),
         uORF = factor(uORF)) -> data_set

summary(data_set) # should only contain numeric data, rownames could be identifiers

#if you're aim is to predict two classes of your data like "dependent vs independent" of a condition, 
#make sure that in your "data_set" group sizes for both classes are nearly equal based on for example p values
#otherwise you might generate a model that is biased towards predicting one or the other class but not the differential
#in other words, if your data contains 90% "independent", your model might be good at predicting "indepedent of the condition" 
#but totally useless for "dependent"
...
gbm.fit2 <- gbm(
  ...
  distribution = "gaussian", # this is basically the type of data of your to be predicted column, gaussian for numerical
  data = data_train,
...
plot <- ggplot(model, aes(fct_reorder(var,rel.inf), fill=fct_reorder(var,rel.inf)))+
  ...
  labs(title="APC/KRAS A2KO", x="feature", y="relative influence")

png(filename = "model.png", width = 500, height = 300)
print(plot)
dev.off()
